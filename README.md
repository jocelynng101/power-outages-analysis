...
# Hypothesis Testing
We decided to highlight the two regions that share the greatest power outage duration difference with a p value < 0.05. Originally, this was 'East North Central' and 'West North Central'.

Once we took a look at the value counts list of the regions, we discovered that 'East North Central' region only had 15 power outages in the cleaned dataset. Beyond that, all of East North Central's pairs had the greatest differences among pairs. Wary of drawing conclusions from minimal data, we elected to avoid using those pairs.

**Null hypothesis:** On average, 'East North Central' and 'Southwest' have equal median durations, and power outages do not vary with respect to those regions.

**Alternative hypothesis:** Power outages in 'East North Central' are longer on average (median) than those from the 'Southwest'.

Our methodology: 
1) The test statistic is the observed difference in medians (median A − median B), compared to a null distribution generated by shuffling region labels within each pair. For calculating the median, we want to avoid "Zero-minute outages" since they are likely errors upon data-insertion.
<br>

2) The first permutation test was for whether outage duration medians differ by climate category. H0: Medians are equal across categories; H1: They differ and p = 0.41, so the observed statistic is **not unusual** under H0. Thus, we fail to reject the null.
<br>

3) The second permutation test was for comparing "normal" vs. all other climate categories. We also tested warm and cold climates for differences. H0: Their median outage durations are equal; H1: Their median outage durations differ. The obtained p-value indicates the observed difference is **common** under H0. We failed to reject the null.
<br>

4) The third permutation test was for variation among climate regions. H0: Regional medians do not differ beyond what random label assignment would produce. H1: The observed spread of regional medians exceeds what random chance would generate. The test statistic was the sum of squared deviations of each region’s median from the global median. The p < 0.05, thus we reject H0 and the observed spread of the regional medians do exceed what the random chance would generate.
<br>

5) The fourth permutation test was for comparing each region’s median to the pooled median of all other regions. For a given region, the test statistic is the absolute median gap: |median(region) − median(rest)|<sup>2</sup>. Under the null, region labels are arbitrary—so we repeatedly shuffle them to form the empirical null distribution. The p-value is the proportion of shuffled statistics >= the observed one. We return a dictionary with base statistics and p values per region for our reference.
<br>

6) Finally, we ran directional permutation tests for all pairs of climate regions (36 tests total).
...
The test statistic is the observed difference in medians (median A − median B), compared to a null distribution generated by shuffling region labels within each pair. The result returned a dictionary with each pair's median difference and p-value.

NOTE: Each permutation test was run with n = 10,000
